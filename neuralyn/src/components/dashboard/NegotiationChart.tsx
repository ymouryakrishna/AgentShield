  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTurnCounter(prevTurn => {
        ...
        const newPrice = 2200 + randomStep;
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-zinc-400 font-medium">
                  {timeframe === 'monthly'
                    ? `${highlightedPoint.label} • ₹${highlightedPoint.price.toLocaleString('en-IN')} Settled`
                    : timeframe === 'weekly'
                    ? `${highlightedPoint.label} Peak • ₹${highlightedPoint.price.toLocaleString('en-IN')} Settled`
                    : `${highlightedPoint.label} • ₹${highlightedPoint.price.toLocaleString('en-IN')} Accepted`}
                </span>
              </div>
            </motion.div>
          )}
      ...
      <div className="pt-4 mt-2 border-t border-white/10 flex flex-wrap items-center justify-between gap-4 text-xs">
        ...
        <div className="flex items-center gap-2 font-mono text-[11px]">
          ...
          <span className="text-cyan-400 font-bold font-mono">
            ₹{avgSettlement.toLocaleString('en-IN')}
          </span>
        </div>
      </div>
    </div>
